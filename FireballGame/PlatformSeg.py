...
class PlatformSeg:
    ...
    def buildStartingArea(self):
        plane = Prism(self.pos, 300, 300, 20, self.white, self.snow, 'start')
        self.parts += (plane,)
    ...
    def platformingOne(self, aux):
        ...
        floatspeed = 5
        meltspeed = .005
        plat_1 = IceCube(LVector3(aux.pos.x + aux.len/2, aux.pos.y + aux.wid * 1.5, aux.pos.z - 10),
        ...
        plat_4 = IceCube(LVector3(plat_3.model.getX() + space*scale, plat_3.model.getY(), plat_3.model.getZ() - 5),
                         scale * 1.5, displacement * 2, floatspeed, meltspeed)
        self.iceCubes += (plat_4,)

        plat_5 = IceCube(LVector3(plat_4.model.getX() + space*scale, plat_3.model.getY(), plat_3.model.getZ()),
                         scale, displacement, floatspeed, meltspeed * 2)
    ...
    def platformingTwo(self, plat):
        ...
        treePlat = Square(LVector3(plat.pos.x + plat.len/2, plat.pos.y, plat.pos.z + 5),
                          500, 100, self.white, self.deadTree, 'treePlat_1')
        treePlat.node.setTransparency(TransparencyAttrib.MAlpha)
        self.parts += (treePlat,)
